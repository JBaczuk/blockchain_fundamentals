...
* `lock time` (4 Bytes) - Indicates either the block height or the timestamp which will make the transaction final. The transaction is not considered complete until this future time.
...
* `previous tx hash` - Hash of the previous transaction. This is used to lookup the transaction in order to validate that this input can be spent.
* `previous utxo index` - Previous Unspent Transaction Output (UTXO) index. Since transactions can have multiple outputs, in order to specify which output, you can provide a zero-based index.
* `scriptSig length` - Length in bytes of the scriptSig.
* `scriptSig` - The first part of the script that is executed before the scriptPubKey from the UTXO. When they are both executed in order, the result must be true in order to be considered a valid spend.
* `sequence number` - Used as a relative lock time if transaction version is >= 2. See [BIP68](https://github.com/bitcoin/bips/blob/master/bip-0068.mediawiki).
...
* `coin value` - `00f2052a01000000`
* `scriptPubKey length` - `43`
